...
<div className="home-top sticky top-0 z-50 navbar">
      ...
      <div className="main-hero flex flex-row justify-between items-center mx-auto">
        <div className="left flex flex-col w-[50%] justify-center mx-auto ">
          ...
          <div className="hero-subtext">
            <p className="mt-6 text-lg text-black/70 outline-font">ApplyForMe helps you land your dream job with AI-powered resume and cover letter optimization, personalized job recommendations, and interview preparation tools.</p>
          </div>
          <div className="cta-actions">
            <div className="email-section mt-8 flex ">
              ...
              <button className="rounded-r-md bg-black text-white px-6 outline-font">Get Started</button>
            </div>
          </div>
          <div className="cta-figures mt-18 flex flex-col gap-6">
            <div className="figures flex flex-col gap-3">
              ...
              <span className="font-bold outline-font text-2xl">500+</span>
              <span className="ml-2 text-black/70 outline-font">Companies using ApplyForMe</span>
              </div>
              <div className="figure-2">
              <span className="font-bold outline-font text-2xl">95%</span>
              <span className="ml-2 text-black/70 outline-font">of users get interviews within 30 days</span>
              </div>
            </div>
    ...
    <div id="whyus" className="feature-section flex flex-col items-center max-w-7xl border border-neutral-200 mx-auto mt-10 mb-10 rounded-xl py-12">
        ...
        <div className="card-1">
          <Card className="bg-neutral-100">
            ...
            <CardContent className="flex flex-col justify-center items-center">
              ...
              <CardDescription className="flex justify-center items-center outline-font text-black/80 text-md mx-auto text-center">
...
function TrustedCompanies() {
  ...
  return (
    <div className="trusted-by-section mt-8 flex flex-col items-center w-full">
      <h3 className="text-sm  text-black/60 mb-4 outline-font">Trusted by companies</h3>
      <hr/>
        ...
        >
          <div className="flex items-center gap-6">
            {companies.map((c) => {
              const Icon = c.icon
              return (
                <div key={c.name} className="min-w-[120px] shrink-0 flex items-center justify-center px-6 py-3 bg-white border border-gray-100 rounded-lg shadow-sm">
                  {Icon ? <Icon size={28} className="text-black" /> : <span className="font-semibold">{c.name}</span>}
                </div>
              )
            })}
          </div>
          ...
          <div className="flex items-center gap-6">
            {companies.map((c, i) => {
              const Icon = c.icon
              return (
                <div key={c.name + '-dup-' + i} className="min-w-[120px] shrink-0 flex items-center justify-center px-6 py-3 bg-white border border-gray-100 rounded-lg shadow-sm">
                  {Icon ? <Icon size={28} className="text-black" /> : <span className="font-semibold">{c.name}</span>}
                </div>
              )
            })}
